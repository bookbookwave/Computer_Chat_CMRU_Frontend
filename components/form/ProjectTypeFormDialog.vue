        ...
        <v-container>
          <v-row>
            <v-col cols="12" sm="12" md="12">
              <v-text-field v-model="data.name" label="Full Project Type name*" hint="example of persistent helper text" persistent-hint required />
            </v-col>
          </v-row>
        </v-container>
...
const onSubmit = () => {
  ...
  if (props2.textDialog === 'New') {
    mutationsDatabase().createProjectTypes({
      onResult: () => {
    ...
    mutationsDatabase().updateProjectTypes({
      onResult: () => {
        window.location.reload()
        queryDatabase({})
        data.dialog = false
