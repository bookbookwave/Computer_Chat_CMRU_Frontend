<template>
  <div>
    <v-no-ssr>
      <v-data-table :headers="data.headers" :items="blogNews" class="elevation-1" :search="data.search">
        <template #top>
          <v-toolbar flat>
...
const blogNews = await useBlogNews().blogNews

blogNews!.forEach((item: any) => {
