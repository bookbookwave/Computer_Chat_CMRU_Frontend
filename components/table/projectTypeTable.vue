<template>
  <div>
    <v-no-ssr>
      <v-data-table :headers="data.headers" :items="types" class="elevation-1" :search="data.search">
        <template #top>
          <v-toolbar flat>
...
const deleteItemConfirm = () => {
  mutationsDatabase().deleteStatus({ onResult: () => { }, value: data.deleteIndex })
        types!.splice(data.editedIndex, 1)
        closeDelete()
}
